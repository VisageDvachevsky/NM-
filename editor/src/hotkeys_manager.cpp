...
std::vector<const ShortcutCommand*> HotkeysManager::getCommandsInCategory(ShortcutCategory category) const
{
    std::vector<const ShortcutCommand*> result;
...
bool HotkeysManager::handleKeyDown(i32 keyCode, bool ctrl, bool shift, bool alt, bool meta)
{
    ...
    if (ctrl) mods = mods | Modifiers::Ctrl;
    if (shift) mods = mods | Modifiers::Shift;
    if (alt) mods = mods | Modifiers::Alt;
    if (meta) mods = mods | Modifiers::Meta;
...
std::string HotkeysManager::keyCodeToString(KeyCode key)
{
    ...
    if (key >= KeyCode::A && key <= KeyCode::Z) {
        return std::string(1, static_cast<char>('A' + (static_cast<i32>(key) - static_cast<i32>(KeyCode::A))));
    }
    if (key >= KeyCode::Num0 && key <= KeyCode::Num9) {
        return std::string(1, static_cast<char>('0' + (static_cast<i32>(key) - static_cast<i32>(KeyCode::Num0))));
    }
...
std::string HotkeysManager::modifiersToString(Modifiers mods)
{
    ...
    if (hasModifier(mods, Modifiers::Ctrl)) result += "Ctrl+";
    if (hasModifier(mods, Modifiers::Shift)) result += "Shift+";
    if (hasModifier(mods, Modifiers::Alt)) result += "Alt+";
...
std::string HotkeysManager::categoryToString(ShortcutCategory category)
{
    switch (category) {
        ...
        case ShortcutCategory::Transform: return "Transform";
...
std::string HotkeysManager::contextToString(ShortcutContext context)
{
    switch (context) {
        ...
        case ShortcutContext::SceneView: return "Scene View";
        case ShortcutContext::StoryGraph: return "Story Graph";
        case ShortcutContext::Timeline: return "Timeline";
        case ShortcutContext::Inspector: return "Inspector";
        case ShortcutContext::AssetBrowser: return "Asset Browser";
        case ShortcutContext::Hierarchy: return "Hierarchy";
        case ShortcutContext::Console: return "Console";
        case ShortcutContext::TextEdit: return "Text Edit";
        case ShortcutContext::NodeEdit: return "Node Edit";
